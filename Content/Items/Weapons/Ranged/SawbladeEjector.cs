...
namespace CCMod.Content.Items.Weapons.Ranged
{
    ...
    public class SawbladeEjectorHeldProj : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 normOrigin = new Vector2(-1 + recoil.X, 12);
    ...
    public class SawbladeEjectorProjectile : ModProjectile
    {
        ...
        float rotVelDir;
        public override bool ShouldUpdatePosition() => ShotOut || hitNPC is null;
        ...
        public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            if (!ShotOut)
            {
                hitDirection = MathF.Sign(target.Center.X - Player.Center.X);
            }
        }
        ...
        public override void Kill(int timeLeft)
        {
            if (!Main.dedServ)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    Gore gore = Gore.NewGoreDirect(Projectile.GetSource_Death(), Projectile.Center + dir, Vector2.Zero, Mod.Find<ModGore>(Name + $"_Gore{1 - i}").Type);
