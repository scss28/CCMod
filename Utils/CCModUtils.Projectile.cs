	...
	partial class CCModUtils
	{
		...
		public static bool LookForProjectile(this Projectile projectile, float distance, int amountofItself = 0)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				...
				{
					if (CompareSquareFloatValue(projectile.Center, Main.projectile[i].Center, distance))
						count++;
					if (count >= 2 + amountofItself)
						return true;
				}
			}
		...
		private static bool CompareSquareFloatValue(Vector2 pos1, Vector2 pos2, float Distance)
		{
			return Vector2.DistanceSquared(pos1, pos2) <= Distance * Distance;
		}
